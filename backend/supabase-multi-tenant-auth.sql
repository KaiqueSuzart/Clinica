-- =====================================================
-- SISTEMA MULTI-TENANT COMPLETO - AUTENTICAÇÃO E ISOLAMENTO
-- =====================================================
...
-- =====================================================
-- Primeiro, verificar se a tabela empresa já existe e qual o tipo do ID
-- Se existir com BIGINT, vamos manter a compatibilidade

-- Criar tabela empresa se não existir
CREATE TABLE IF NOT EXISTS empresa (
  id BIGSERIAL PRIMARY KEY,  -- Usando BIGSERIAL para compatibilidade
...
CREATE TABLE IF NOT EXISTS usuarios (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  empresa_id BIGINT NOT NULL REFERENCES empresa(id) ON DELETE CASCADE,  -- Mudado para BIGINT
  nome VARCHAR(255) NOT NULL,
  ...
  auth_user_id UUID UNIQUE -- Referência ao usuário do Supabase Auth
);
...
COMMENT ON FUNCTION user_belongs_to_empresa(UUID, UUID) IS 'Verifica se usuário pertence à empresa';
COMMENT ON FUNCTION create_user_with_empresa(UUID, UUID, VARCHAR, VARCHAR, VARCHAR, VARCHAR) IS 'Cria usuário e associa à empresa';
